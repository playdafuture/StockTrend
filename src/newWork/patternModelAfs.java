...
public class patternModelAfs {
    ...
    public patternModelAfs(DataPointArray originalDPA, String stockName) {
        trArray = new Vector<Trade>();
    ...
    private void detectPattern() {
        if (instanceDPA.hasXRelatives(5)) {
            if (instanceDPA.getLastRelative().getFlag() > 0) {
                ...
                if (instanceDPA.getXthRelative(1).getFlag() == -1 &&
                ...
                //Last flag is low, test for LHLHL pattern to set up long
                if (instanceDPA.getXthRelative(1).getFlag() == 1 &&
                        instanceDPA.getXthRelative(2).getFlag() == -2 &&
                        instanceDPA.getXthRelative(3).getFlag() == 2) {
    ...
    private void setupLong() {
        ...
        double stopLoss = 0;
        stopLoss = instanceDPA.getXthRelative(2).getLow() - 0.01;
    ...
    private void setupShort() {
        ...
        Trade t = new Trade(name);
        double stopLoss = 0;
        stopLoss = instanceDPA.getXthRelative(2).getHigh() + 0.01;
