...
public class patternModelD1fs {
    ...
    public patternModelD1fs(DataPointArray originalDPA, String stockName) {
    ...
    private void detectPattern() {
        if (instanceDPA.hasXRelatives(5)) {
            if (instanceDPA.getLastRelative().getFlag() > 0) { //last is high: set up short
                if (latestHigh != null && latestHigh.getDate().isEqual(instanceDPA.getLastRelative().getDate())) {
                    return;
                }
                if (instanceDPA.getXthRelative(4).getFlag() == +2 &&
                        instanceDPA.getXthRelative(3).getLow() < instanceDPA.getXthRelative(1).getLow() &&
                        instanceDPA.getXthRelative(2).getHigh() <= instanceDPA.getXthRelative(0).getHigh()) {
                    latestLow = instanceDPA.getLastRelative();
                    setupShort();
                }
            } else { //last is low: set up long
                ...
                if (instanceDPA.getXthRelative(4).getFlag() == -2 &&
                        ...
                        instanceDPA.getXthRelative(2).getLow() >= instanceDPA.getXthRelative(0).getLow()) {
                    latestLow = instanceDPA.getLastRelative();
                    setupLong();
                }
            }
    ...
    private void setupLong() {
        ...
        double stopLoss = instanceDPA.getXthRelative(4).getLow() - 0.01;
    ...
    private void setupShort() {
        ...
        Trade t = new Trade(name);
        double stopLoss = instanceDPA.getXthRelative(4).getHigh() + 0.01;
