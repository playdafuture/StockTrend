...
public class targetModel {
    ...
    public targetModel(DataPointArray originalDPA, String stockName, double rf) {
    ...
    public Vector<Trade> simulate() {
            ...
            if (tempDP.getLow() >= instanceDPA.getLast().getLow() && tempDP.getHigh() <= instanceDPA.getLast().getHigh()){
                ...
                if (trArray.isEmpty() || !lastTrade().isOpen()) { //no trade or trade is not open
                    ...
                    if (tempDP.getLow() <= lastTrade().stopLoss) {
                        ...
                        if (lastTrade().isOpen()) {
                            lastTrade().Close(instanceDPA.getLast().getDate(), instanceDPA.getLastSL().getLow() - 0.01);
                        }
                        currentTrend = trend.unknown;
                    }
                    //end of up trend
                    ...
                    if (tempDP.getHigh() >= lastTrade().stopLoss) {
                        ...
                        if (lastTrade().isOpen()) {
                            lastTrade().Close(instanceDPA.getLast().getDate(), instanceDPA.getLastSH().getHigh() + 0.01);
                        }
                        currentTrend = trend.unknown;
                    }
                    //end of down trend
        ...
        } else { //trade is only setup, remove from list
    ...
    private void trendUp() {
        ...
        if (!trArray.isEmpty() && lastTrade().isOpen()) { //in a trade already
            ...
            if (lastTrade().direction != Direction.longTrade) {
                System.out.println("ERROR! Trade should be long here!!");
            }
    ...
    private void trendDown() {
        ...
        if (!trArray.isEmpty() && lastTrade().isOpen()) { //in a trade already
            //trade direction checker
            if (lastTrade().direction != Direction.shortTrade) {
                System.out.println("ERROR! Trade should be short here!!");
            }
